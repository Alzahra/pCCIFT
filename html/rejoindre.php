<!DOCTYPE html>
	<html lang="fr">
		<?php include('entete.php');?>
		<body style="background: url('../Resources/harpe-extr.png') no-repeat fixed center; background-size: cover;">
		    
            <div class="container">
            	<div style="text-align: center; margin-bottom: 10px;">
            		<img src="../Resources/join3.jpg" width="10%">
            	</div>
            	<div style="text-align: center;">
            		<img class="img-responsive" src="../Resources/join2.jpg" width="100%">
            	</div>
            	<nav id="titreJoin">
	            	<h2 >Vous avez des projets ?</h2>
                	<h2>Vous êtes une entreprise Française ou Tunisienne qui désire développer en France ou en Tunisie ?</h2>
                	<h2>Vous êtes un particulier qui veut participer ou soutenir notre objectif ?</h2>
                	<h2>Devenez membre vous êtes les bienvenus.</h2>
                </nav>
                <div id="contenuObjectifs">
                	<ul>
                		<li>Tarif cotisation Membre personne individuelle :</li>
						<li>Tarif cotisation Entreprises :</li>
						<li>Tarif association ou organisation :</li>
                	</ul>
					                    
	            </div>
	            <div class="glyphicon glyphicon-envelope pull-right" style="font-style: italic; font-size: 1.4em; color: black;">
	            	Contactez-nous :
	            	<span><a href="mailto:adhesion@ccift.fr">adhesion@ccift.fr</a></span>
            	</div>
            </div>
		        <!-- /#page-content-wrapper -->
		 <?php include('pied.php'); ?>
    	<script src="https://code.jquery.com/jquery-3.2.1.js"></script>
    	<script src="../js/bootstrap.min.js"></script>
    	<script src="../js/site.js"></script>

	</body>

</html>